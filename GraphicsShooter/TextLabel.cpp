...
TextLabel::TextLabel(std::string text, std::string font, glm::vec2 _position, bool _positionIsCenter)
{
	this->positionIsCenter = _positionIsCenter;
	this->text = text;
	...
	program = shaderLoader.CreateProgram("Shaders/text.vert", "Shaders/text.frag");
...
int TextLabel::getWidth()
{
	...
	for (c = text.begin(); c != text.end(); c++)
	{
		...
		widthSum += static_cast<int>((ch.Advance >> 6) * this->scale);
	}
